    ...
    fn interpolate_variable_in_command() {
        assert_eq!(
            ...
            Configuration::new(
                [(
                    "bar".into(),
                    ir_explicit_build("0", vec!["bar".into()], Rule::new("42", ""), vec![]).into()
                )]
                .into_iter()
                .collect(),
    ...
    fn interpolate_two_variables_in_command() {
        assert_eq!(
            ...
            Configuration::new(
                [(
                    "bar".into(),
                    ir_explicit_build("0", vec!["bar".into()], Rule::new("1 2", ""), vec![]).into()
                )]
                .into_iter()
                .collect(),
                ["bar".into()].into_iter().collect(),
    ...
    fn interpolate_dollar_sign_in_command() {
        assert_eq!(
            ...
            Configuration::new(
                [(
                    "bar".into(),
                    ir_explicit_build("0", vec!["bar".into()], Rule::new("$", ""), vec![]).into()
                )]
                .into_iter()
                .collect(),
                ["bar".into()].into_iter().collect(),
    ...
    fn interpolate_out_variable_in_command() {
        assert_eq!(
            ...
            Configuration::new(
                [(
                    "bar".into(),
                    ir_explicit_build("0", vec!["bar".into()], Rule::new("bar", ""), vec![]).into()
                )]
                .into_iter()
                .collect(),
                ["bar".into()].into_iter().collect(),
        ...
        assert_eq!(
            ...
            Configuration::new(
                [
                    (
                        "bar".into(),
                        ir_explicit_build("0", vec!["bar".into()], Rule::new("", ""), vec![])
                            .into()
                    ),
                    (
                        "baz".into(),
                        ir_explicit_build("1", vec!["baz".into()], Rule::new("", ""), vec![])
                            .into()
                    )
                ]
                .into_iter()
        ...
        fn reference_variable_in_parent_module() {
            ...
            assert_eq!(
                ...
                Configuration::new(
                    [(
                        "bar".into(),
                        ir_explicit_build("0", vec!["bar".into()], Rule::new("42", ""), vec![])
                            .into()
                    )]
                    .into_iter()
                    .collect(),
