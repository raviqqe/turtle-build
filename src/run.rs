...
pub async fn run(
    ...
    let priorities = sort_builds(&config, &builds);
...
async fn spawn_build(context: Arc<RunContext>, build: Arc<Build>) -> Result<(), BuildError> {
    spawn(async move {
                    ...
                    .get_hash(HashType::Content, build.id())?
        {
            ...
            let output = run_rule(&context, rule, context.priority(build.id())).await;
...
mod tests {
    ...
    #[test]
    fn assign_priorities_to_unknown_builds() {
        let context = RunContext::new(
            create_context(
                &Default::default(),
                &Default::default(),
        ...
        assert_eq!(context.priority(pool_build("foo", None).id()), 0);
        assert_eq!(context.priority(pool_build("bar", None).id()), 1);
        assert_eq!(context.priority(pool_build("baz", None).id()), 2);
    }

    #[tokio::test]
    async fn run_waiting_commands_by_priority() {
        let command_runner = FakeCommandRunner::default();
        let context = RunContext::new(
            create_context(&command_runner, &Default::default(), &Default::default()),
            create_pool_config(vec![], &[], &[]),
